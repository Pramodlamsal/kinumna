<div class="card-header bg-white py-3">
    <h5 class="heading-6 mb-0">{{__('Select Nearest Pick-up Point')}}</h5>
</div>
@php
...
@if (!empty($admin_products))
    ...
    <label class="mega-radio w-100">
        ...
        <span class="d-block">
            <br><strong>{{ __('Address') }}: {{ $pick_up_point->name }}</strong>
            <br><strong>{{ __('Address') }}: {{ $pick_up_point->address }}</strong>
            <br><strong>{{ __('Phone') }}: {{ $pick_up_point->phone }}</strong>
        </span>
    </label>
    @endforeach
@endif
